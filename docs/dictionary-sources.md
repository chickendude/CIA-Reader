...
|---|---|---|---|
| [Hindi WordNet](http://www.cfilt.iitb.ac.in/wordnet/webhwn/) | CFILT, IIT Bombay | Research use, distributable with attribution | **Importer landed (T-3.10a, 2026-05-05)** — TSV-per-synset format (`<id>\t<cat>\t<concept>\t<example>\t<words>`); each unique (word, POS) becomes one lemma, `source_id = hwn:<synset>:<word_idx>`. Dump requires CFILT registration; place at `apps/web/data/dictionaries/hindi-wordnet/synsets.tsv` and run `pnpm dictionary:import hindi-wordnet`. Concept stored as Hindi-target translation row (monolingual dump); cross-language linkage via synset id is a future ticket. |
| [Dbnary Hindi-English](http://kaiko.getalp.org/about-dbnary/) | GETALP / Univ. Grenoble Alpes | CC-BY-SA 3.0 | **Imported (T-3.10b, 2026-05-05)** — fetched via `scripts/fetch-dictionary-sources.sh dbnary-hi` (Turtle dump, decompressed in place); streaming Turtle subset parser groups triples by `LexicalEntry` IRI, takes `ontolex:writtenRep` (or `rdfs:label` fallback) as headword, `lexinfo:partOfSpeech` for POS, and `skos:definition`@en on linked senses as the translation set |
...
| *Molesworth's A Dictionary, Marathi and English* (1857) | Public domain (DSAL) | Public domain | **Importer landed (T-3.10d, 2026-05-05)** — TEI-Lex0 XML; chunked entry-block scanner pulls `<orth>`/`<pos>`/`<def>` per `<entry>`. NFC-only orthographic normalizer ships now; the curator-reviewed archaic-spelling fix-up table is a separate follow-up (the importer's `OrthographicNormalizer` interface is the single drop-in once the table is signed off). `source_id = molesworth:<xml:id or raw archaic spelling>` so re-imports stay idempotent across normalizer changes. |
...
| OdiaNLP resources | Community / various | Mixed (MIT / CC-BY) | **Importer landed (T-3.10g, 2026-05-05)** — JSONL at `apps/web/data/dictionaries/odianlp/curated.jsonl`, one entry per line with `{id, headword, pos, definitions, publisher, license}`. License gate is enforced in the parser: any entry with a license outside the allowlist (MIT, CC0-1.0, CC-BY-3.0/4.0, CC-BY-SA-3.0/4.0, PublicDomain) raises `OdiaNlpLicenseError` and stops the run. Per-entry attribution combines publisher + license (e.g. `OdiaNLP / Subhadarshi (CC-BY-4.0)`) so the credits page can group correctly. |
