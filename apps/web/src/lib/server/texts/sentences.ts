 ...
 * Used to capture the sentence a word was mined from (Anki export) and to feed
 * the OpenAI sentence translator.
 */
...
type MiniToken = { idx: number; surface: string };

/**
 * Reconstruct the sentence containing `tokenIdx` from an ordered token list:
 * the run from just after the previous sentence-ender through the next one
 * (inclusive), with surfaces concatenated (gap tokens preserve spacing).
 */
export function sentenceFromTokens(tokens: MiniToken[], tokenIdx: number): string {
  ...
  let start = pos;
  while (start > 0 && !SENTENCE_END_RE.test(ordered[start - 1]!.surface)) start -= 1;
